  ...
  return (
    <li className="qLi" key={question.id}>
      ...
      <span className="datePosted">
        Posted on <Moment format="YYYY/MM/DD">{question.created_date}</Moment>{" "}
        by {question.user_name}
      </span>
      {props.answers
        .filter((answer) => answer.question == props.question.id)
        .map((answer) => (
      ...
      {question.author === props.userID && (
        <button
          onClick={() =>
            props.handleDeleteQuestion(question.id, question.author)
          }
        >
          Delete
        </button>
